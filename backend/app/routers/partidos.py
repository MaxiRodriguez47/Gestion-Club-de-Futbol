...
def crear_partido(partido: PartidoCreate, db: Session = Depends(get_db), admin_actual=Depends(requiere_admin)):
...
def cargar_fixture(partidos: List[PartidoCreate], db: Session = Depends(get_db), admin_actual=Depends(requiere_admin)):
...
def obtener_partido(partido_id: int, db: Session = Depends(get_db), usuario_actual=Depends(get_current_user)):
...
def actualizar_partido(partido_id: int, partido: PartidoUpdate, db: Session = Depends(get_db), admin_actual=Depends(requiere_admin)):
...
def registrar_resultado(partido_id: int, resultado: RegistrarResultado, db: Session = Depends(get_db), admin_actual=Depends(requiere_admin)):
...
def eliminar_partido(partido_id: int, db: Session = Depends(get_db), admin_actual=Depends(requiere_admin)):
