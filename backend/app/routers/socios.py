...
def listar_socios_morosos(db: Session = Depends(get_db)):
...
def registrar_pago(socio_id: int, pago: RegistrarPago, db: Session = Depends(get_db)):
...
def dar_de_baja_socio(socio_id: int, db: Session = Depends(get_db)):
